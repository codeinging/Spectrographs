...
public class UVCCamera {
    ...
    public UVCCamera() {
		...
		Logger.w("mNativePtr:"+mNativePtr);
    	mSupportedSize = null;
	}

    /**
     * connect to a UVC camera
     * USB permission is necessary before this adb method is called
     * @param ctrlBlock
     */
    public void open(final UsbControlBlock ctrlBlock) {
    	...
    	if (mNativePtr != 0 && TextUtils.isEmpty(mSupportedSize)) {
			...
			Logger.w("mSupportedSize:"+mSupportedSize);
    	}
		int a=nativeSetPreviewSize(mNativePtr, DEFAULT_PREVIEW_WIDTH, DEFAULT_PREVIEW_HEIGHT,
			DEFAULT_PREVIEW_MIN_FPS, DEFAULT_PREVIEW_MAX_FPS, DEFAULT_PREVIEW_MODE, DEFAULT_BANDWIDTH);
		Logger.w("nativeSetPreviewSize："+a);
    }
	...
	public Size getPreviewSize() {
		...
		Logger.w("getPreviewSize:"+result.toString());
		return result;
	}
    ...
    public void setPreviewDisplay(final SurfaceHolder holder) {
   		int a=nativeSetPreviewDisplay(mNativePtr, holder.getSurface());
		Logger.w("nativeSetPreviewDisplay:"+a);
    }
    ...
    public void setPreviewDisplay(final Surface surface) {
    	int a=nativeSetPreviewDisplay(mNativePtr, surface);
		Logger.w("nativeSetPreviewDisplay:"+a);
    }
    ...
    public void setFrameCallback(final IFrameCallback callback, final int pixelFormat) {
    	if (mNativePtr != 0) {
        	int a=nativeSetFrameCallback(mNativePtr, callback, pixelFormat);
			Logger.w("nativeSetFrameCallback:"+a);
    	}
    }
    ...
    public void startPreview() {
    	if (mCtrlBlock != null) {
    		int a=nativeStartPreview(mNativePtr);
			Logger.w("nativeStartPreview:"+a);
    ...
    public void stopPreview() {
    	...
    	if (mCtrlBlock != null) {
    		int a=nativeStopPreview(mNativePtr);
			Logger.w("nativeStopPreview:"+a);
...
//================================================================================
	public synchronized void setAutoFocus(final boolean autoFocus) {
    	if (mNativePtr != 0) {
    		int a=nativeSetAutoFocus(mNativePtr, autoFocus);
			Logger.w("nativeSetAutoFocus:"+a);
    	}
    }
...
//================================================================================
	...
	public synchronized void setFocus(final int focus) {
    	if (mNativePtr != 0) {
			...
			int a = 0;
 		   if (range > 0)
 			    a=nativeSetFocus(mNativePtr, (int)(focus / 100.f * range) + mFocusMin);
				Logger.w("nativeSetAutoFocus:"+a);
    	}
    }
	...
	public synchronized int getFocus(final int focus_abs) {
	   int result = 0;
	   if (mNativePtr != 0) {
		   int a=nativeUpdateFocusLimit(mNativePtr);
		   Logger.w("nativeUpdateFocusLimit:"+a);
		   final float range = Math.abs(mFocusMax - mFocusMin);
		   if (range > 0) {
...
//================================================================================
	...
	public synchronized void setBrightness(final int brightness) {
    	if (mNativePtr != 0) {
			...
			int a=0;
 		   if (range > 0)
 			   a=nativeSetBrightness(mNativePtr, (int)(brightness / 100.f * range) + mBrightnessMin);
			Logger.w("nativeSetBrightness"+a);
    	}
    }
	...
	public synchronized int getBrightness(final int brightness_abs) {
	   int result = 0;
		int a=0;
	   if (mNativePtr != 0) {
		   a=nativeUpdateBrightnessLimit(mNativePtr);
		   Logger.w("nativeUpdateBrightnessLimit"+a);
		   final float range = Math.abs(mBrightnessMax - mBrightnessMin);
		   if (range > 0) {
	...
	public synchronized void resetBrightness() {
		int a=0;
    	if (mNativePtr != 0) {
    		a=nativeSetBrightness(mNativePtr, mBrightnessDef);
			Logger.w("nativeUpdateBrightnessLimit"+a);
    	}
    }
